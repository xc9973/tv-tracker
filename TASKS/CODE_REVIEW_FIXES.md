...
### Task 13: 资源时间自动推断尊重用户手动覆盖 ✅
- **优先级**: 🟡 中
- **文件**:
  - `internal/service/task_generator.go`
  - `internal/service/show_sync.go`
  - `internal/service/subscription.go`
- **问题**: 在 TMDB 数据刷新时，可能无条件用 `InferResourceTime` 覆盖用户手动设置的 `ResourceTime`
- **风险**: 中 - 用户手动设置会被“自动推断”回写，造成配置丢失
- **修复方案**:
  - 引入/落实 `ResourceTimeIsManual` 作为保护开关
  - 仅在 `ResourceTimeIsManual=false` 时自动推断并写回
  - 仅在 `OriginCountry` 变化时触发自动推断
- **状态**: ✅ 已完成

### Task 14: 属性测试与缓存驱动同步行为对齐 ✅
- **优先级**: 🟡 中
- **文件**:
  - `tests/property/subscription_test.go`
  - `tests/property/task_generator_test.go`
- **问题**:
  - Subscription 测试仅允许 `/tv/{id}`，但订阅会触发季同步 `/tv/{id}/season/{n}`
  - TaskGenerator `SyncAll` 依赖缓存数据，但测试未预热缓存导致 falsify
- **修复方案**:
  - mock server 同时处理 details 与 season endpoints
  - 在相关属性测试中调用 `cacheSvc.Refresh(tmdbID)` 预热缓存
- **状态**: ✅ 已完成
